...
newarray.map(element=>{dataaa.push(element._id)})
          https://ophtaback.onrender.com+'/updatefile',{
  method:'PATCH',
  ...
  'Content-type': 'application/json; charset=UTF-8'
    ...
    useEffect(
       ()=>{
         https://ophtaback.onrender.com+'/fileDattente').then(response=>response.json().then(data=>{setfs(data.patient);set_all_patients(data.patient)}))
      },[])
