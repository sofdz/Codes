...
export default function SupprimerPatient({selected,deleting,setDeleting}) {
    ...
    const handleConfirmDelete = () => {
        fetch('https://ophtaback.onrender.com/patients/', {
          method: 'DELETE',
    ...
    return (
        <Dialog on PaperProps={{ sx: { alignItems: 'center', minHeight: '135px', minWidth: '400px' } }} open={deleting}>
            <Typography sx={{ m: 2, alignSelf: 'center' }} variant='overline' >Voulez vous supprimer ({selected.length}) patient(s) séléctionné(s)?</Typography>
