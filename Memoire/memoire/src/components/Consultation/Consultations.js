...
export default function Consultation({ openAddConsult, setOpenAddConsult,patient,setnewConsult,setnew}) {
    const {user}=useAuth()
    const addConsultaion=(e)=>{
        if(!actes_consult.length){setOpen_acte(true);return}  
           e.preventDefault()
       var consultation={patient:patient._id,
        medecin:user._id,
    ...
    fetch('https://ophtaback.onrender.com/createConsult', {
        method: 'POST',
        body: JSON.stringify(consultation),
        headers: {
            "Content-Type": 'application/json'
        }
    }).then(res =>res.json().then(data=>{{console.log(data);setOpenAddConsult(false);setnewConsult(data);setnew(true)}})).catch(e => console.log(e))
       }
    const handleClose = () => {
        setOpenAddConsult(false);
    };
    const [actes_consult,setActesConsult]=useState([])
    const [RefractionG,setRefractionG]=useState({S:0,C:0,A:0})
    const [CorrectionG,setCorrectionG]=useState({S:0,C:0,A:0})
    const [infosG,setinfosG]=useState({TO:0,PCH:0,LAF:" ",FO:" ",addition:0})
    const [RefractionD,setRefractionD]=useState({S:0,C:0,A:0})
    const [CorrectionD,setCorrectionD]=useState({S:0,C:0,A:0})
    const [infosD,setinfosD]=useState({TO:0,PCH:0,LAF:" ",FO:" ",addition:0})
    ...
    const [motif_DP,setMD]=useState({motif:"",distance:0})
                ...
                <AppBar position='static'>
                    <Toolbar>
                        <Grid container direction='row' justifyContent='space-between' alignItems='center'>
                            ...
                            <Grid item>
                                <Typography component='h3' style={{left:'auto'}}> Ajouter une nouvelle consultation </Typography>
                            </Grid>
                            <Grid item>
                    ...
                    <Typography variant='h6' style={{color:'#000000'}}>Informations de la Consultation: </Typography>
                        ...
                        {checkDilatation&&<Card variant='outlined'>
                            <CardContent>
                                ...
                                <TextField sx={{marginTop:'10px'}} label='remarque' value={dilatInfo.remarque_dil} onChange={e=>{e.preventDefault();setdilatInfo(prevs=>({...prevs,remarque_dil:e.target.value}))}}  fullWidth multiline maxRows={4} ></TextField>
                            </CardContent>      
                            </Card>}
                            <Stack direction='row' justifyContent='center' alignItems='center'   divider={<Divider orientation="vertical" flexItem />}>
                                ...
                                <AjouterActes open_acte={open_acte} setOpen_acte={setOpen_acte} actConsult={actes_consult} setActesConsult={setActesConsult} />
                            </Stack>
                    </Paper>
