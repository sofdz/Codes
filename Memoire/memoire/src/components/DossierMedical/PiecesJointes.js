...
export default function PiecesJointes({ patient }) {
            ...
            renderCell: (params) => (
                <>
                    <IconButton onClick={() => {
                        ...
                        https://ophtaback.onrender.com+'/supprimerFichier/'.concat(patient._id).concat('/').concat(params.row.name),urlParams)
    ...
    const handleFileChange = (e) => {
        ...
        https://ophtaback.onrender.com+'/ajouterFichier/'.concat(patient._id).concat('/').concat(cpt), urlParams).then(res => res.json().then((data) => {
            console.log(data)
        }))
    }
