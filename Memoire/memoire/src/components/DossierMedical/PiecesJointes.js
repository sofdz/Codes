...
export default function PiecesJointes({ patient }) {
            ...
            renderCell: (params) => (
                <>
                    <IconButton onClick={() => {
                        ...
                        fetch('/supprimerFichier/'.concat(patient._id).concat('/').concat(params.row.name),urlParams)
    ...
    const handleFileChange = (e) => {
        ...
        fetch('/ajouterFichier/'.concat(patient._id).concat('/').concat(cpt), urlParams).then(res => res.json().then((data) => {
            console.log(data)
        }))
    }
