...
export default function Ordonnance({ consultation, open, setOpen, patient }) {
    ...
    useEffect(() => {
        ...
        fetch('/ordonnances/'.concat(consultation._id)).then(res => res.json().then((data) => {
            setOrdonnances(data)
            if (data.length > 0) {
    ...
    const validerOrdonnance = () => {
        ...
        if (newOrd) {
            ...
            fetch('createdOrd', urlParams).then(res => res.json().then((data) => {
                //refreshOrd()
            ...
            fetch('ordonnance/'.concat(ordonnance._id), urlParams).then(res => res.json().then((data) => {
